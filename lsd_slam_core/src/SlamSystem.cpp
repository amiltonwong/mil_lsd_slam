...
void SlamSystem::changeKeyframe(bool noCreate, bool force, float maxScore)
{
	...
	std::shared_ptr<Frame> newKeyframeCandidate = latestTrackedFrame; // 最近tracked frame 作為new Keyframe的候選
	if(doKFReActivation && SLAMEnabled)
	{
		struct timeval tv_start, tv_end;
		gettimeofday(&tv_start, NULL);
		// new Keyframe的候選 從過往的一系列的keyframe 中進行查找比對, 比對成功的過往keyframe存下到 "newReferenceKF" 變量
	...
	if(newReferenceKF != 0) // 比對成功, 可以過往keyframe "newReferenceKF" 讀進至 NewCurrentKeyframe
		loadNewCurrentKeyframe(newReferenceKF);
	else // 比對不成功(没有符合), 需创建一个新的关键帧
	{
		if(force)
		{
			if(noCreate)
...
// SE3Tracking's main funtion
// uchar* image: input image
// store the tracked pose to ""
void SlamSystem::trackFrame(uchar* image, unsigned int frameID, bool blockUntilMapped, double timestamp)
{
	...
	// 要求： 參考幀必需為current的keyframe
	// 2. 更新参考帧：如果当前参考帧不是最近的关键帧(current keyframe)，则更新参考帧
	if(trackingReference->keyframe != currentKeyFrame.get() || currentKeyFrame->depthHasBeenUpdatedFlag)
	{
		trackingReference->importFrame(currentKeyFrame.get());
		currentKeyFrame->depthHasBeenUpdatedFlag = false; // as current keyframe is just added as reference frame, its depth map has not been refined by succedding tracked images
		trackingReferenceFrameSharedPT = currentKeyFrame;
	}
	...
	// 3. 初始化位姿：把上一帧(previous keyframe frame)与参考帧(the current keyframe as ref frame)的位姿当做初始位姿
	// pose-pose constraint (motion) is sim3, so , get the se3 from sim3
	SE3 frameToReference_initialEstimate = se3FromSim3(
			// current keyframe pose (K_i) * the previous keyframe pose (K_(i-1)) -> relative pose
			// left multification operation
			// inverse(T_w,i) * T_w,i-1 => T_i,i-1 (relative pose from K_(i-1) to K_i)
	...
	SE3 newRefToFrame_poseUpdate = tracker->trackFrame(
			...
			frameToReference_initialEstimate); // pass 初始位姿
	...
	// 5. 判断是否跟踪失败"TRACKING LOST"：根据跟踪的像素点个数多少以及跟踪质量来判断
	// INITIALIZATION_PHASE_COUNT = 5
	...
	// Keyframe selection
	// 6. 关键帧筛选：通过计算得分确定是否构造新的关键帧 eqt.(4.16), 代码中有两处构建新关键帧的地方。一个是正常系统运行的时候，一个是在重定位的时候。
	latestTrackedFrame = trackingNewFrame;
	// MIN_NUM_MAPPED = 5
	// 重定位部分
	// 第一个if判断的条件是当前建图线程是否已经更新好上一帧关键帧以及跟踪最近的关键帧（参考帧）的图像帧个数不小于一定值（MIN_NUM_MAPPED，为5）。如果满足上述的条件则计算得分，得分大于一定阈值则确定构建新的关键帧
	if (!my_createNewKeyframe && currentKeyFrame->numMappedOnThisTotal > MIN_NUM_MAPPED)
	{	// pose graph中的pose 作比較， 為補償scale因子， 需要除以mean depth 來歸一化
		// newRefToFrame_poseUpdate 指 \xi_(j,i)
		Sophus::Vector3d dist = newRefToFrame_poseUpdate.translation() * currentKeyFrame->meanIdepth; // normalize by dividing by mean depth of current keyframe
		// 需要注意，在计算运动距离dist的时候，这里采用位移向量先乘以一个平均场景逆深度：大场景的逆深度会小，小场景的逆深度会大，这相当于一个权重，对于相同的位移，认为小场景的运动程度比较大。

		// 阈值变量 minVal
		float minVal = fmin(0.2f + keyFrameGraph->keyframesAll.size() * 0.8f / INITIALIZATION_PHASE_COUNT,1.0f); // INITIALIZATION_PHASE_COUNT = 5
		if(keyFrameGraph->keyframesAll.size() < INITIALIZATION_PHASE_COUNT)	minVal *= 0.7; // empirical
		// 这里的两个操作都是使得初始化阶段当关键帧比较少的时候（INITIALIZATION_PHASE_COUNT为5）放宽了阈值，之后就是１

        // 计算得分: 得分和当前帧和参考帧之间的位移大小有关,  tracker->pointUsage 可以理解为当前帧和参考帧重叠区域的比例
		lastTrackingClosenessScore = trackableKeyFrameSearch->getRefFrameScore(dist.dot(dist), tracker->pointUsage);
		// minVal is used as the selection criteria
		if (lastTrackingClosenessScore > minVal)
		{
			createNewKeyFrame = true; // flag indicating to select this frame as keyframe

			if(enablePrintDebugInfo && printKeyframeSelectionInfo)
				printf("SELECT %d on %d! dist %.3f + usage %.3f = %.3f > 1\n",trackingNewFrame->id(),trackingNewFrame->getTrackingParent()->id(), dist.dot(dist),
                       tracker->pointUsage, trackableKeyFrameSearch->getRefFrameScore(dist.dot(dist), tracker->pointUsage));
		}
		else
		{
			if(enablePrintDebugInfo && printKeyframeSelectionInfo)
