...
public:
	...
	float fx,fy,cx,cy;
	float fxi,fyi,cxi,cyi; // inverse value
...
private:
	...
	float* buf_d; // inverse depth
	float* buf_idepthVar; // variance of inverse depth
	float* buf_weight_p; // weight for photometric error??

	int buf_warped_size;

    // compute error term
	float calcResidualAndBuffers(
			const Eigen::Vector3f* refPoint, // 3D point position for ref image
			const Eigen::Vector2f* refColVar, //??  pixel variance for ref image
			int* idxBuf,
			int refNum,
			Frame* frame,
			const Sophus::SE3f& referenceToFrame,
			int level, // pyramid level : lvl3 : 80x60, lvl2 : 160x120, lvl1 : 320x240 , lvl0 : 640x480
			bool plotResidual = false); // useful for displaying residual
	...
	void calculateWarpUpdate( // update
			LGS6 &ls);
#if defined(ENABLE_SSE)
	...
	float affineEstimation_a_lastIt;  // used to append "affine lighting change" for I_ref
